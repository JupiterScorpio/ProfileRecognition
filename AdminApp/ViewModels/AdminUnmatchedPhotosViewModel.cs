...
{
    public class AdminUnmatchedPhotosViewModel : BaseViewModel
    {
        private long totalPageSize = 0;
        private int records = 0;

        public AdminUnmatchedPhotosViewModel()
        {
            GetUnmatchedPhotos();
        }
        ...
        {
            get { return new DelegateCommand(NavigateToUnmatchedPhotosPage); }
        }
        public ICommand MatchedPhotosCommand
        {
            get { return new DelegateCommand(NavigateToMatchedPhotosPage); }
        }
        ...
        private List<UnmatchedPhotosResponseModel> _unmatchedPhotosList;

        public List<UnmatchedPhotosResponseModel> UnmatchedPhotosList
        {
            get { return _unmatchedPhotosList; }
            set { _unmatchedPhotosList = value; OnPropertyChanged("UnmatchedPhotosList"); }
        }
        ...
        private async void PreviousButton()
        {
            ...
            if (PageNo >= 1)
            {
                ...
                response = await StaticContext.GetUnMatchApiUrl.PostAsync<ResponseBody>(page);
                if (response.IsValidated)
                {
                    //List<AllPhotosResponseModel> list = new List<AllPhotosResponseModel>();
                    List<UnmatchedPhotosResponseModel> list = JsonConvert.DeserializeObject<List<UnmatchedPhotosResponseModel>>(response.Data);
                    UnmatchedPhotosList = list;
                }
                else
                {
        ...
        private async void NextButton()
        {
            ...
            if (totalPageSize > records)
            {
                ...
                if (response.IsValidated)
                {
                    ...
                    List<UnmatchedPhotosResponseModel> list = new List<UnmatchedPhotosResponseModel>();
                    list = JsonConvert.DeserializeObject<List<UnmatchedPhotosResponseModel>>(response.Data);
                    UnmatchedPhotosList = list;
                }
                else
                {
        ...
        public void NavigateToUnmatchedPhotosPage() => rootFrame.Navigate(typeof(AdminUnmatched));
        public void NavigateToMatchedPhotosPage() => rootFrame.Navigate(typeof(AdminMatched));
        private async void GetUnmatchedPhotos()
        {
            AdminPagination page = new AdminPagination();
            try
            {

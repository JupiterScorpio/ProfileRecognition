...
namespace Repository
{
    ...
    public static class Extension
    {
        ...
        public static async Task<string> FileUpload(string id,byte[] img ,string name)
        {
            ...
            if (bInit)
            { 
                string imgPathname = Path.Combine(path, fileName);
                bool ret = FaceMatch.FaceCompare.MakeImageEmbedding(imgPathname);
            ...
            {
                path = null;
                throw new ApplicationException("FaceMatching failed");
            }
